...
个人本地工具：导入面试录音转写文本 → 自动切分「面试官问题 / 我的回答」（忠实原文、可溯源）→
单题复盘（优化建议独立一栏）→ 按知识点跨场去重合并进面经库 → 导出 Markdown。

> 当前为 **Phase 3 最小闭环骨架**（方案二：Python + Flask + Jinja2 + 原生 JS + SQLite）。
> 产品与设计文档见 `docs/`。
...
浏览器打开 http://127.0.0.1:8000 → 「导入复盘」→ 点「载入示例文本」即可在**无任何 API Key**
的情况下跑通完整闭环（规则/mock 提取）。离线冒烟测试：`python scripts/smoke_e2e.py`（28 项断言）。
